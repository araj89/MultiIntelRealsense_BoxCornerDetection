...
def post_process_depth_frame(depth_frame, decimation_magnitude=1.0, spatial_magnitude=2.0, spatial_smooth_alpha=0.5,
    ...
    #spatial_filter.set_option(rs.option.holes_fill, 5)
    temporal_filter.set_option(filter_smooth_alpha, temporal_smooth_alpha)
    temporal_filter.set_option(filter_smooth_delta, temporal_smooth_delta)

    # Apply the filters
    #filter_frame = hole_filter.process(depth_frame)
...
class DeviceManager:
    def __init__(self, context, pipeline_configuration, exposure = 2000, gain = 16, laser_power = 270):
        """
    ...
    def enable_device(self, device_serial, enable_ir_emitter):
        ...
        sensor.set_option(rs.option.exposure, self._exposure)
    ...
    def enable_auto_exposure(self, State = True, roi = None):
        for (device_serial, device) in self._enabled_devices.items():
            sensor = device.pipeline_profile.get_device().first_depth_sensor()
            sensor.set_option(rs.option.enable_auto_exposure, State)

            sensor2 = device.pipeline_profile.get_device().first_roi_sensor()

            #roi = sensor2.get_region_of_interest()
            """roi = rs.region_of_interest()
            roi.min_x = 300
            roi.min_y = 300
            roi.max_x = 600
            roi.max_y = 600
            sensor2.set_region_of_interest(roi)"""
